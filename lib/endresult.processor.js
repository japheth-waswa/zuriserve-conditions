...
export const processEndResultVarGroup = ({ contentString }) => {
  try {
    ...
    // build images
    const { fields: buildImages } = interpolateStringExpressions({
      str: parsedContentString,
      dataTags: IMAGE_BUILDER_OPENING_CLOSING_TAGS_UTIL,
      requestFields: true,
    });
    ...
    const { fields: queryObjects } = interpolateStringExpressions({
      str: parsedContentString,
      dataTags: QUERY_OBJECT_OPENING_CLOSING_TAGS_UTIL,
      requestFields: true,
    });
