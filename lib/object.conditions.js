...
export const processConditionalStatements = ({
  ...
  try {
    ...
    } else if (lodash.isString(data) && data.trim().length > 0) {
      dataExists = true;
      data = data.trim().toLowerCase(); //it string,then to lowercase
    } else if (data) {
      dataExists = false;
    }
    ...
    // console.log("---dt---");
    // console.log({operator:conditionalStatement.operator, conditionalStatementValue, data });
