...
const largeDatasetFieldBuilder = ({
  ...
  try {
    ...
    const data = objectLabelData.value.data || [];

    if(lodash.isEmpty(data))return largeDatasetFieldBuilder({
      feeObject,
      objectLabelsData,
      largeDatasetFieldTriggers,
      processedObject,
    ...
    //parse ids
    const parsedIds = records
      .filter((item) => (item.id || item._id ? true : false))
      .map((item) => item.id || item._id);

    processedObject[objectLabelId] = {
      ids: [...processedObject[objectLabelId].ids, ...parsedIds],
      fields: [
        ...processedObject[objectLabelId].fields,
        largeDatasetFieldName.toLowerCase(),
      ],
    };
...
const processFormulaeParams = ({ feeObject, objectLabelsData }) => {
  try {
    // extract trigger objects for normal fields
...
const processFeeObject = ({
  ...
  try {
    ...
    // console.log(util.inspect(`conditionalStatus = ${conditionalStatus}`, false, null, true));
