...
import {
  ...
  QUERY_OBJECT_FIELD_VALUE_DECORATOR,
  ...
  try {
    ...
    for (const queryItem of query) {
      ...
      if (queryItem.matchType === QUERY_BUILDER_MATCH_TYPES.TERM.value) {
        ...
        if (
          lodash.isUndefined(queryItem.value) ||
          (lodash.isString(queryItem.value) &&
            lodash.isEmpty(queryItem.value.trim()))
        )
          continue;
        ...
        value = lodash.isString(queryItem.value)
          ? queryItem.value.trim()
          : queryItem.value;
      } else if (
        queryItem.matchType ===
        ...
        value = objectLabelData.value.data;
      } else if (
      ...
      if (
        ...
        (!lodash.isNumber(value) && !lodash.isEmpty(value))
      ) {
        let parsedValue;
        //check if decorator set
        if (!lodash.isUndefined(queryItem.objectFieldValueDecorator)) {
          try {
            if (
              QUERY_OBJECT_FIELD_VALUE_DECORATOR.START_OF_DAY.value ===
              queryItem.objectFieldValueDecorator
            ) {
              parsedValue = moment(value).startOf("day").format(DB_DATE_FORMAT);
            } else if (
              QUERY_OBJECT_FIELD_VALUE_DECORATOR.END_OF_DAY.value ===
              queryItem.objectFieldValueDecorator
            ) {
              parsedValue = moment(value).endOf("day").format(DB_DATE_FORMAT);
            }
          } catch (e) {}
        }
        //value exists
        queryData.push({ ...queryItem, value: parsedValue || value });
      }
    }
